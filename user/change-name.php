...
if(!empty($_POST))
{
	...
	if(count($errors) == 0)
	{
		$loggedInUser->updateUsername($username);
	}
} ?>
		...
		<div id="main">
            <div id="regbox">
				...
				<div style='position:relative; top:290px; right:10px; text-align:right; font-family:Verdana'>
                    <input type="submit" value="Update Name" class="submit" />
				</div>

				Yellow Pages
				<h1><?php echo $lang["CHANGE_NAME"]?></h1>

                <p>
                    <label><?php echo $lang["CHANGE_NAME"]?>:</label><br>
                    <input type="text" name="username" style="width:200px" value="<?php echo $loggedInUser->display_username; ?>" />
                </p>
                </form>
            </div>
